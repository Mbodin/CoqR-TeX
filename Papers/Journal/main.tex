...
\documentclass[
    ...
    natbib=false % Note: This is here to be able to use Biber.
 ]{acmart}
%\let\citename\relax

\settopmatter{printfolios=true} % ,printacmref=false}

% Note: We should receive the exact information after publishing.
% \setcopyright{acmlicensed}
% \acmPrice{15.00}
% \acmDOI{10.1145/3276945.3276946}
% \acmYear{2020}
% \copyrightyear{2020}
% \acmISBN{}
% \acmConference[TOPLAS '20]{Proceedings of the 14th ACM SIGPLAN International Symposium on Dynamic Languages}{November 6, 2018}{Boston, MA, USA}
...
% % \acmBooktitle{DLS'18, November 6, 2018, Boston, MA, USA}

% \startPage{1}
% \pagestyle{empty} % We are not supposed to have any page number.
...
\renewcommand*{\bibfont}{\footnotesize} % We are allowed to set the font size of the bibliography to anything between 8pt (\footnotesize) to 10pt (\normalsize), (9pt is \small).
...
% See https://tex.stackexchange.com/questions/336041/remove-n-d-no-date-from-online-entries-without-dates-in-biblatex
% There is no need for a “nodate” in online websites.
...
\affiliation{
  ...
  % \postcode{Post-Code2a}
  % \country{Chile}
  \titlenote{T. Diaz and É. Tanter are partially funded by the Millenium Institute for Foundational Research on Data (IMFD Chile)}
}
...
% Note: Due to a bug of the ACM package, this section does not appear.
...
\keywords{R, Coq, Testing infrastructure}

\maketitle
% \thispagestyle{empty} % To compensate the \thispagestyle{plain} of \maketitle to get unnumberred pages as requested.
...
Indeed, there are many ways to call a function, and in particular there are two ways to provide an argument: either by position or by name (including prefixes). %\footnote{
...
Additionally, we found several mismatches between the text description and the behavior of the reference interpreter, GNU~R~\parencite{Rwebsite}.\footnote{For instance, according to the language definition, \mintinline{R}{if ("TRUE") 42} should raise an error, whilst the interpreter returns \mintinline{R}{42}.}
...
\begin{sloppypar}
Second, we have developed a testing framework that streamlines the process of running both CoqR and GNU~R on a set of test cases and report on mismatches and errors, among others (Section~\ref{sec:testing:architecture}). We use several test suites to measure the advancement of the CoqR implementation.
...
A \mintinline{Coq}{string} is provided to help debugging.
    \end{sloppypar}
%
...
\newtext{(see Section~\ref{sec:fuel} for more details)}.
\end{itemize}
...
\newtext{when calling \mintinline{C}{setjmp},
a default value is provided to continue computations;
when a result of the form \mintinline{Coq}{result_longjump} reaches its corresponding \mintinline{C}{setjmp},
the continuation of \mintinline{C}{setjmp} is run again, using 
the value carried by \mintinline{Coq}{result_longjump} to replace the default one.}
%This binder was surprisingly easy to define in Coq.
This obliviousness to global state, errors, \newtext{jumps,} and non-termination is what enables a close correspondence between a program written in both languages.
...
\end{minted}
\vspace{-1em} % The last line of the code is mostly white.
...
that is, pointers to {\em basic language elements} (see Section~\ref{sec:heap}).}
...
Other differences are that several arguments are systematically passed to each function (\mintinline{Coq}{globals}, \mintinline{Coq}{runs}, and \mintinline{Coq}{S}),
the use of \mintinline{Coq}{read%list}
instead of \mintinline{C}{CAR} in the C version,
as well as the absence of the \mintinline{C}{PROTECT} macro in the Coq version.
We clarify each of these points in the following subsections.
...
Inductive SEXPRECData :=
  ...
  (* ... *).
Coercion listExp : ListStruct >-> SEXPRECData.@\label{line:SEXPREC:Coercion:listExp}@
...
then the constructor \mintinline{Coq}{listExp} will be implicitly added
thanks to Line~\ref{line:SEXPREC:Coercion:listExp} of Figure~\ref{fig:SEXPREC}.
In the context of CoqR,
this is more than a simple syntactic notation
as it helps the eyeball correspondence
by hiding in the Coq code what is not present in C.
%
...
closely mimicking the unguarded accesses of C.
...
\mintinline{C}{R_FunTab}, which is the {\em symbol table} that maps operation and function names to their C implementions.
...
In fact \mintinline{C}{do_attr_formals} is a {\em hidden} global variable, introduced locally with the C \mintinline{C}{static} keyword inside the \mintinline{C}{do_attr} function.
...
the code initializes the \mintinline{C}{static} variable \mintinline{C}{do_attr_formals},
...
Consequently, we treat \mintinline{C}{static} variables just like global variables.
...
\begin{figure}
\begin{minted}[breaklines=false]{Coq}
...
we define \mintinline{Coq}{runs} of type \mintinline{Coq}{Runs}
by linking these functions to themselves.
We define \mintinline{Coq}{runs} using fuel:
...
% }
% %
% In addition to passing the extra \mintinline{Coq}{runs} argument,
...
We choose not to model the garbage collection aspect of GNU~R in CoqR. 
\newtext{A garbage collector is an optimization of a language implementation, which is arguably irrelevant for the specification of the language itself (for instance, neither R6RS for Scheme nor ECMAScript for JavaScript even mention garbage collection).}
% Indeed, garbage collection adds complexity to the interpreter, but is supposed to not affect the result of any computation. \newtext{
% Consequently, the reasoning enabled by CoqR is to be understood ``up to a correct GC'', which is a reasonable assumption. Verifying the GNU~R GC is a separate project.}
...
Currently, CoqR only considers ASCII strings, and hence does not support various locales and character encodings supported by R.
...
Interestingly, all syntactic constructs correspond to a C function. This includes constructs like \mintinline{R}{if}, \mintinline{R}{while}, \mintinline{R}{return}, and even assignments:
...
it reads a global boolean variable %\mintinline{C}{eatLines}
...
After interpretation of the R code, we obtain the raw outputs for each interpreter, GNU~R and CoqR. % , which are just strings.
...
\caption{Current results of running the different test suites.
    ...
    {\protect\centering\footnotesize (P = Pass, F = Fail, NI = Not Implemented, NF = Not Found, \newline I = Impossible, U = Unknown, PP = Potential Pass, PF = Potential Fail)}}
\label{tab:results}
    %\hspace*{-3mm}
...
For example, the \mintinline{R}{eval} function sets default values for the arguments of the internal \mintinline{R}{eval} function:
\begin{minted}[breaklines=false]{R}
...
To evaluate this expression, the \mintinline{R}{atan} function has to be implemented.
...
Table~\ref{tab:results} presents the current status of executing the four test suites described earlier with CoqR.
...
Note that no constraint is given for the first element \mintinline{Coq}{car} of the list: lists in R are heterogeneous.
%
...
% % it would for instance not be productive to rewrite the same equality in both directions,
% leading to the same context we started with.
...
%
\changed{In the conclusion of the lemma, the \mintinline{Coq}{result_prop} predicate
specifies what the resulting state of \mintinline{Coq}{do_attr} satisfies
in the interesting cases
(ignoring cases such as \mintinline{Coq}{result_bottom})}.
...
This novel tactic discharges the premises of the lemma and introduces the resulting state:
...
The formalization is composed of two parts: a mechanized specification and an interpreter. The JSCert specification is syntactically related with the ECMAScript specification through an eyeball correspondence, and the interpreter passes its test suite.
...
a similar tool for R is an interesting future work.}
...
%
% This is to be mirrored with the fact that CompCert and Formalin did not reuse GCC's source code for C,
% which would have been unusable.


\section{Conclusions}
\label{sec:conclusion}

We present the first formal specification of R, written as an interpreter in Coq. We present the monadic encoding that allows CoqR to be in eyeball correspondence with the GNU~R reference interpreter, provide a testing infrastructure useful to incrementally enrich CoqR, and show that CoqR can be used to formally reason about R. Considering the number of realistic test cases that CoqR currently passes, and its extensible architecture, we believe that CoqR can be grown to fully cover R and its main libraries. 
\newtext{Among the different venues for future work, it would be particularly interesting to reduce the shim further, by moving the parser and the pretty-printer to Coq.} On the formal reasoning side, 
CoqR should be the basis on top of which to develop an appropriate program logic, to more conveniently reason about R programs. As such, it is a necessary first step towards a robust environment for formal verification of R programs. 
