...
\end{minted}
    ...
    %               % to too much space.
    \caption{The original C function \mintinline{C}{applyClosure}}
    \label{fig:c:applyClosure}
\end{figure}
...
\end{minted}
    \caption{The Coq translation of \mintinline{C}{applyClosure}}
    \label{fig:coq:applyClosure}
\end{figure}

\mb{Another interesting example}
\begin{figure}
\begin{minted}{C}
EXP* attribute_hidden do_attr (EXP* call, EXP* op, EXP* args, EXP* env){
  EXP* argList, ans;
  int nargs = R_length (args);
  PROTECT (argList = matchArgs(do_attr_formals, args, call));
  if (nargs < 2 || nargs > 3)
    error ("Either 2 or 3 arguments are required.");
  argList_car = CAR (argList);
  /* ... */
  return ans;
}
\end{minted}
    % \vspace{-2mm} % There are too much blank in the last line of the code, leading
    %               % to too much space.
    \caption{The original C function \mintinline{C}{do_attr}}
    \label{fig:c:do_attr}
\end{figure}

\begin{figure}
\begin{minted}{Coq}
Definition do_attr S (call op args env : EXP_pointer) : result SEXP_pointer :=
  ...
  ifb nargs < 2 \/ nargs > 3 then
    ...
    read%list argList_car, argList_cdr, argList_tag := argList using S in
    (* ... *)
    result_success S ans.
\end{minted}
    \caption{The Coq translation of \mintinline{C}{do_attr}}
...
\begin{minted}{C}
  ...
  if (formals == NULL)
    formals = allocFormalsList2 (install ("x"), install ("which"));
\end{minted}
    \caption{Another snippet of \mintinline{C}{do_attr}}
...
\end{minted}
    \caption{The Coq translation of the code of Figure~\ref{fig:c:do_attr:formals}}
