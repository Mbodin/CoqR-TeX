...
We manually analyzed the reported failed results (5.7\%) and observed that most are due to mismatches of double numbers precision and string encodings\td{~40\% double mismatch, barely 3\% strings, the rest is a mix}. We are currently exploring the actual discrepancies, as well as finding ways to eradicate these mismatches.
%\et{can you get a number here? how many are in fact precision/strings issues?}

10.7\% of test cases end up classified as ``unknown'', which subsequently lead to potential passes and fails (5\%)\td{Actually, around 80-90\% (al ojo) of potential fails are due to missing objects, caused by failed assignments}. Recall that these are due to the various ways in which GNU~R's pretty printer works, which \CoqR{} currently does not mimic properly. However, the relatively low percentage of these cases does not seriously question the validity of the implementation. Fixing these unknown results is tiresome, but not conceptually challenging.
...
In Coq, this translates into a large number of intermediate states.
Some invariants are automatically discharged by the \mintinline{Coq}{computeR} tactic,
but not all of them.
Typically, if the invariants are temporarily broken during the execution of a function,
...
Proving that the invariants hold for such functions
involve to prove that everything has been set back into places.
This typically involve following all possible paths
from the newly allocated objects and show that the invariants hold for each of them.
...
and more than a thousand lines for the lemmae used by these tactics.
The compiled file for the proofs about our 20 simple functions
is however larger than the one containing all the lemmae used by the tactics:
