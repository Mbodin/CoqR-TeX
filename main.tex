...
the test suite of GNU~R itself\footnote{https://github.com/wch/r-source/}, and two test suites from the FastR project~\parencite{kalibera2014fast}, which we refer to as FastR-1\footnote{https://github.com/h2oai/fastr} and FastR-2\footnote{https://github.com/oracle/fastr}. These external test suites are first curated to eliminate expressions which use certain features\td{Do I say how many are left out?}. For instance, tests which require internet connection, read or write files, or generate charts. Additionally, some tests require specific packages (for example, the JIT package) or use particular templates in Java to generate cases. In total, these curated test suites include more than 20,000 tested expressions\td{Do I have to be more precise?}.
...
In addition to these test suites, we have developed our own test suite, specifically tailored for testing corner cases of the language. It is also meant to be used as a faster way of testing the implemented features, during the development process\td{Felt it was a bit empty without the other paragraph haha}. Our own test suite, referred to as CornerCases, features over 2,000 tested expressions\td{No, the previous corner case was not true anymore}.
...
This step is carried out by {\em result processors} (implemented in Python). There are two classes of processors; one that handles GNU~R outputs, and one that handles CoqR outputs.  Each processor matches the received raw string against a list of regular expressions (regexes). As soon as a regex matches, an object of type \mintinline{Python}{Result} is created and the matching stops. There are nine subclasses of \mintinline{Python}{Result}:
 ...
 \mintinline{Python}{ListResult} and \mintinline{Python}{VectorResult}, with further subclasses: \mintinline{Python}{BooleanVectorResult}, \mintinline{Python}{NumericVectorResult} and \mintinline{Python}{StringVectorResult}.
...
Unsurprisingly, our initial experiments running the test suites of GNU~R and FastR raised a huge number of undefined function errors, whose consolidated report allowed us to locate in the base library of R. As anecdote, having implemented 105 features from the symbol table, more than 80\% of the failures for the GNU~R test suite were still due to missing definitions\td{64\% of all results as well}. After extending \CoqR{} to be able to load the base library, that percentage fell down to about 40\% \td{I feel percentages do not show the change... At first it was 1000/1200 and it fell to 300/700}, corresponding to other basic libraries, most importantly the statistics library.
...
\tdi{Mhh... Do you use a particular package to show tables? And should I include the potentials + unknown results?}

\begin{table}[]
\centering
\caption{Current results obtained executing the test suites.\footnote{Execution time from a 2-core, Intel(R) Xeon(R) CPU E5-2650 v4 @ 2.20GHz, 4 GB RAM machine  }}
\label{my-label}
\begin{tabular}{|l|l|l|l|l|l|}
\hline
Test Suite  & Pass & Fail & Not Implemented & Impossible & Time {[}s{]} \\ \hline
GNU R       & 234  & 29   & 744             & 1          & 5,000          \\ \hline
FastR1      & 1044 & 65   & 988             & 5          & 6,000          \\ \hline
FastR2      & 2485 & 2155 & 5827            & 7          & 45,000         \\ \hline
CornerCases & 2513 & 8    & 56              & 0          & 6,000          \\ \hline
\end{tabular}
\end{table}
