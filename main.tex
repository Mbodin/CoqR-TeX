...
in one of 7 classes\td{Should we leave this as it is? It is still true, but seeing the figure, it seems like it's not} (Figure~\ref{fig:testing:interface}):
...
shown in Figure~\ref{fig:result}\td{I actually had it separate from this because it looks too cluttered. Should I just leave it like this (for the purpose of the picture)?}.
...
Most of the failed results are due to double precision or string encoding. There are however real discrepancies in certain cases, which are already reported to be fixed.
...
many cases as  ``unknown'' (leading to potential passes and failures in the next steps). Reducing this number could be a little tricky, given the different printing modes.
