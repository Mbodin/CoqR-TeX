...
Once a single step in a test file fails, or leads to a not-implemented result,
we cannot say anything about what the steps that follow in the test file: both errors and success could coincidental.
...
we use the potential pass and fail outcome categories. These two categories mean nothing for correctness (we cannot conclude anything certain from a potential outcome), but they really help in the development process by showing possible future results.
...
Below is an example:
the defined function \mintinline{R}{eval} behaves very similarly
to the internal \mintinline{R}{eval} function.
The difference is that \mintinline{R}{eval} checks the value of its parameters
when determining their default values.
This definition extends the original simpler behavior of the function.
\tdi{Don't know how to make it look prettier...}
\begin{minted}{R}
...
to define the function \mintinline{R}{eval}. Because the internal \mintinline{R}{eval}
function is already implemented, it is safe to use the previously defined one.
In some other cases the internal functions are not implemented, which results in
a not-implemented result if one actually calls the function defined in the base library.
...
On the other hand,
if we had not executed the base library,
then the interpreter might have failed with an error---%
stating for instance that the \mintinline{R}{eval} function
does not exist---%
