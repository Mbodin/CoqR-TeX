...
Indeed, the base library is crucial in order to execute the test suites of both GNU~R and FastR. For instance, having implemented 105 features from the symbol table, for the GNU~R (resp. FastR) test suite, more than 80\% (resp. 60\%) of the failed results are due to missing definitions
contained in this library. Barely 3\% are caused by actual value mismatch, which is often caused by double precision or string encoding. The remaining percentage corresponds to missing definitions due to previous errors in computation.

It is possible to see an immediate change after partially including the base library\footnote{Only 3 out of 159 files left out.}.
With the same 105 implemented features from the symbol table, for the GNU~R (resp. FastR),
there are no longer failed results due to missing definitions.\td{From 880 failed results it ends up with 500. There are still missing functions from other basic libraries (stats)} Instead, the amount
of not-implemented results increases from 200 up to 800 (resp. 780 to X).
...
Loading the base library into \CoqR{}'s environment requires around 15 hours.
...
the base library increases up to 60h. The test suites increase to X and Y, respectively.\td{have to check times... Apparently I forgot to write them down...}
%
% \section{Old}
% \subsection{Results}
