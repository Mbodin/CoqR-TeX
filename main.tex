...
\end{figure}
\noindent\textbf{Two-step comparison.} 
To compare each result, we adopt the general scheme shown in Figure~\ref{fig:methodology}. In particular, the comparison of outcomes proceeds in two steps. First,
...
errors, boolean vectors, numerical vectors, etc. as well as the special 
...
Sometimes typing a result fails. 
...
the way vectors are printed, and our pretty-printer is not aware of all of these special attributes. In such cases, the typing step fails
returning ``unknown'' as a type. Such tests are consequently marked as ``unknown''; they could be functionally correct or not, so classifying them apart is practical.
