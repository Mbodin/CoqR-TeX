...
Additionally,\et{update once finished} we report on the process of scaling up our interpreter in order to be able to load existing R libraries (Section~\ref{sec:library}), in particular the R base library. Finally, we report on a proof effort to establish that memory invariants are preserved during execution, including some specific proof automation (Section~\ref{sec:proofs}).
%
...
the test suite of GNU~R itself\footnote{https://github.com/wch/r-source/}, and two test suites from the FastR project~\parencite{kalibera2014fast}, which we refer to as FastR1\footnote{https://github.com/h2oai/fastr} and FastR2\footnote{https://github.com/oracle/fastr}. These external test suites are first curated to eliminate expressions which rely on external effects and resources, such as internet connection, access files, and generate charts.
 Additionally, other excluded tests require specific packages (for example, the JIT package) or use particular templates in Java to generate cases. In total, these curated test suites include more than 20,000 tested expressions.\et{say how many are left out} Overall, as far as we were able to observe, all these test suites have very little overlap.\et{is that true?}
...
This step is carried out by {\em result processors} (implemented in Python). There are two classes of processors; one that handles GNU~R outputs, and one that handles CoqR outputs.  Each processor matches the received raw string against a list of regular expressions (regexes). As soon as a regex matches, an object of type \mintinline{Python}{Result} is created and the matching stops. There are nine subclasses of \mintinline{Python}{Result}:
 ...
 \mintinline{Python}{ListResult} and \mintinline{Python}{VectorResult}, with further subclasses for boolean, numeric and string vector results.
...
% \eti{update with ``not found'' -- update screenshot as well}
...
in one of 7 classes\td{Should we leave this as it is? It is still true, but seeing the figure, it seems like it's not}\et{to be updated once we've have definitely converged on the "good way" to present} (Figure~\ref{fig:testing:interface}):
...
shown in Figure~\ref{fig:result}\td{I actually had it separate from this because it looks too cluttered. Should I just leave it like this (for the purpose of the picture)?}\et{I don't understand what you mean}.
...
Unsurprisingly, our initial experiments running the test suites of GNU~R and FastR raised a huge number of undefined function errors, whose consolidated report allowed us to locate in the base library of R. As anecdote, having implemented 105 features from the symbol table, around 1,000 of 1,200 failures
...
\hline
{\bf Test Suite}  & {\bf P} & {\bf F} & {\bf NI} & {\bf NF} & {\bf I}
& {\bf U} & {\bf PP} & {\bf PF} \\ \hline\hline
Corners     & 2513 & 8    & 56      &   233     & 0    & 134 & 42 & 5 \\ \hline
GNU R       & 234  & 29   & 744     &   737     & 1    & 19 & 0 & 0 \\ \hline
FastR1      & 1044 & 65   & 988     &    1    & 5    & 175 & 61 & 98 \\ \hline
FastR2      & 2504 & 2157 & 6355    &   1224     & 7    & 575 & 437 & 217 \\ \hline
\end{tabular}
...
\eti{describe the results}

Without considering the Corners test suite, most of the results are of the type Not Implemented. This is an expected result, considering the incremental development being carried out and that there are currently only 112\td{Update} out of 700 features implemented from the symbol table, described earlier.

Most of the failed results are due to double precision or string encoding. There are however real discrepancies in certain cases, which are already reported to be fixed.\et{can you get a number here? how many are in fact precision/strings issues?}

The Not Found results are also along the lines of what was expected. The basic libraries are being partially loaded as required by the test suites, in order to reduce execution times. This means that new functions may be needed after implementing new features due to allowing new code to be executed. Following an iterative process, these definitions can be incorporated as per case.

The Impossible results are bugs from \CoqR{} which are reported and being worked on.
...
many cases as  ``unknown'' (leading to potential passes and failures in the next steps). Reducing this number could be a little tricky\et{what does this mean?}, given the different printing modes.
...
With the current results and with the tools provided by our testing framework, we believe that given more engineering power, it is possible to scale up to the full language and its main libraries.
