...
the test suite of GNU~R itself ($\sim$3,600 lines)
% \td{There are 3600 lines but it's really ~1700 expressions. Is that ok?}
and the test suite of FastR ($\sim$1,300 lines)~\parencite{kalibera2014fast}.
% \td{Same here, 1300 lines but actually ~2600 expressions}.
We found that these test suites mainly focus on {\em expected} behaviors.
For instance,
...
%
As mentioned before, if \CoqR{} were bug free, an ``impossible'' result would correspond to a critical bug in GNU~R---in practice, all the ``impossible'' results we observed were due to mistakes in \CoqR{} and were quickly solved.
...
The base library corresponds to a group of files written in R,
totalizing 19,000 lines of code, which are executed by GNU~R prior to any other
...
Most of these definitions do not require many features in order to be run: only
...
of the base library contains the \mintinline{R}{pi} definition, shown below.
...
This library along with a few others (such as the stats library) prepare the base
environment needed for GNU~R to run properly.

\subsection{Importance of including the base library}\td{Meh... }

The need to include this library into CoqR becomes clear when executing the test suites.
For the GNU~R test suite, more than 80\% of the failed results are due to missing definitions
contained in this library.  Barely 3\% are caused by actual value mismatch, which
is often caused by double precision or string encoding. The remaining percentage
corresponds to missing definitions due to previous errors in computation.
This distribution of failed results is similar for the fastR test suite. Around 60\%
are a consequence of missing the base library, 3\% is value mismatch and the rest
is as in the case for GNU~R.

Recognizing the need for the base library is made possible by the use of one of
our frameworks' tools.
This tool allows for an easy visualization of the most common cases where evaluation
fails due to missing functions, as can be seen in figure \ref{fig:library:missing_functions}.
...
\begin{figure}
    \includegraphics[width=.5\textwidth]{missing_functions_base_r.png}
    \caption{Chart displaying the most needed function definitions.}
    \label{fig:library:missing_functions}
\end{figure}
...
\td{Didn't know how to write it in present tense...}
...
\td{Don't know where to put this but I think it is important to make the distinction}
It is important to make the distinction at this point between being able to run the definitions
and being able to use them. The previous sections describe the process of including
the base library to CoqR. This means, being able to execute the definitions and include
them in its environment. However, this does not mean that they are usable.
Indeed, the internal functions used by many of them may not yet be implemented.
Hence, calling them would result in a not-implemented result.
This type of behaviour should be expected and profit from it.

The main issue of not having the base library's functions and objects is not being able to detect
the actual features required by the test suites. By removing the layer of failed
results due to missing definitions, it is possible to focus the efforts on implementing
what is really needed. We include tools in our framework that show the most
...
we use the Bisect coverage tool for OCaml.
