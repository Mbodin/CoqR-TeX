...
\affiliation{
  ...
  % \postcode{Post-Code2a}
  % \country{Chile}
  \thanks{{\'E.} Tanter is partially funded by the Millenium Institute for Foundational Research on Data (MI-FRD)}
}
...
Second, we have extensively {\em tested} the Coq interpreter against the GNU~R reference interpreter, using three test suites: the test suite of GNU~R itself ($\sim$3,600 loc), the test suite of the FastR implementation ($\sim$1,300 loc)~\parencite{kalibera2014fast}, and a custom test suite that we developed ourselves to exercise corner cases ($\sim$1,100 loc).
...
Finally, we report on the process of scaling up our interpreter in order to be able to import existing R libraries (Section~\ref{sec:library}), in particular the R base library ($\sim$19,000 loc).
...
but instead of a recusrive call,
...
a testing framework that supports comparative execution of \CoqR{} and GNU~R. The framework additionally helps the development process by providing useful reports.
...
We based ourselves on two existing test suites:
the test suite of GNU~R itself ($\sim$3,600 lines)\td{There are 3600 lines but it's really ~1700 expressions. Is that ok?}
and the test suite of FastR ($\sim$1,300 lines)~\parencite{kalibera2014fast}\td{Same here, 1300 lines but actually ~2600 expressions}.
We found that these test suites mainly focus on expected behaviors.
For instance,
different numerical vectors are concatenated,
summed, divided, etc.
In each case,
the special values \mintinline{R}{NaN} and \mintinline{R}{NA}
are checked to propagate.
But we did not find any test checking for instance
how the \mintinline{R}{"TRUE"} string is converted into a boolean value,
despite its behavior being non-trivial:
...
\mb{Maybe we should put a screenshot after executing a large test suite instead of this one.}
...
shown in Figure~\ref{fig:result}. Discriminating a failure from a  not-yet-implemented feature is crucial for development, as it helps us choose which feature to focus on next (see Section~\ref{sec:library:development}).

As mentioned before, an impossible result would correspond to a critical bug in GNU~R---in practice, all the ``impossible'' results we observed were due to mistakes in \CoqR{} and were quickly solved.
The last three categories are explained later on.\\
...
and does not follow an eyeball correspondence with GNU~R's pretty-printer, which is quite complex.
This introduces harmless output mismatches. For instance, when printing vectors
with GNU~R, some are left padded, others are right padded.
...
Sometimes processing a result fails.
...
the way vectors are printed, and our pretty-printer is not aware of all of these special attributes. In such cases, the processing step fails returning ``unknown'' as a type. Such tests are consequently marked as ``unknown''; they could be functionally correct or not, so classifying them apart is practical.
...
Once a single step in a line-by-line test file fails, or leads to a not-implemented result,
we cannot say anything about the steps that follow in the test file: both errors and success could be coincidental.
...
Prior to executing any expression, GNU~R performs several actions.
One of these is executing the base library, which corresponds to a group of files written in R,
totalizing 19,000 lines of code. This base library includes the definition of several
functions and objects, such as \mintinline{R}{mean}, \mintinline{R}{matrix} or \mintinline{R}{pi}.
Several of these functions are just links to internal functions with some argument
checking or default arguments. An example is the \mintinline{R}{eval} function which
behaves very similarly to the internal \mintinline{R}{eval} function.
The only difference is that the base library function sets default values for the arguments.
\begin{minted}{R}
...
\subsection{Loading the base library}
\label{sec:library:loading}

The first step in loading the base library was fixing the parsing errors that arose,
due to the differences mentioned in section \ref{sec:parsing}. One very common error was
caused by the syntax used for \mintinline{R}{if else} in the R files, considered
a mistake by GNU~R's own documentation\footnote{By typing \texttt{?Control} in R you get a man page with the warning.}.
The files were thus rewritten in order to remove these parsing errors.

Once this was ready, the definitions had to be included. Most of these
are easily run: only the \mintinline{R}{function} keyword is necessary for them
to be assigned. There are other cases where some computation
is required prior to defining the functions or objects.
For instance, the file \texttt{constants.R}
of the base library contains the \mintinline{R}{pi} definition, shown below\td{Don't really know if this is needed}.
In order
...
implemented by our interpreter. In particular, there were three files which
...
It is important to notice that despite being able to perform these assignments and
include the base library to our interpreter, that does not mean that they are usable.
Indeed, the internal functions used by them may not be implemented,
hence calling them would result in a not-implemented result.
We expected this type of behaviour and hoped to profit from it using our framework.
The main issue of not having the base library's functions and objects was not being able to detect
the actual features required by the test suites. By removing the layer of failed
results due to missing definitions, we could focus our efforts on implementing
what was really needed. We included tools in our framework that would show the most
common not-implemented features, allowing us to precisely identify the low-hanging fruits.
This would then allow us to drive the development process forwards.
...
end with a perspective on program logics
