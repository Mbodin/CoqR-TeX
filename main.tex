...
Bugs in certified softwares can still appear.
The reason is that most of the time,
the software is not entirely formalized in Coq.
Some additional code is added,
usually to perform basic inputs and outputs.
This additional code is called the \emph{shim},
and usually concentrate most of a certified software
bugs~\parencite{Yang:2011:FUB:1993498.1993532}.

\todo{}

Issues with parsing.
How the parser itself is in a line-to-line correspondence with the original R parser.
Why this still doesn't prevent us from difference in behavior.

% Parsing in the shim.
Parsing real-world programming languages is known not to be straightforwards.
We wanted our parser to be as close as possible to GNU~R's parser.
Unfortunately, the Bison grammar of GNU~R does not respect
the grammar constraints to be implemented in Menhir directly in Coq~\parencite{jourdan2012validating}.
We instead produced an OCaml parser using Menhir,
which is thus part of the shim.

% Ambiguities in the grammar.
To provide more trust to our parser, we decided to reuse the exact same grammar
than GNU~R, without optimizing it in any way.
This is a first issue with our parser:
the Bison grammar on which GNU~R is based is ambiguous.
As a consequence, Bison makes some arbitrary choices,
and Menhir may take some other.
%
Another source of difference in behavior between the two parser
(GNU~R's and CoqR's) is due to side effects between the lexer and the parser.
Indeed, there are some places in which new lines are significant
and other where they are not\todo{An example}.
