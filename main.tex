...
FastR2      & 2504 & 2157 & 6355  &   1224   & 7   & 575  &   437 & 217 \\ \hline\hline
{\bf Total} & 6285 & 2259 & 8143  &   2195   &  13 & 903  &   540 & 320 \\
\hline
\end{tabular}
{\em total number of tests:} 20,658
...
Unsurprisingly, 50\% of the negative results are either ``not implemented'' or ``not found''. This was to be expected considering that we have currently implemented only 112 \td{Update} out of the 700 features of the symbol table (Section~\ref{sec:coq:structure}), and that only part of libraries are loaded for running the test suites in order to keep execution times under control.\et{this is weird: what would the results be if you would import everything you could?}
...
We manually analyzed the reported failed results (11\%) and observed that most are due to mismatches of double numbers precision and string encodings. We are currently exploring the actual discrepancies, as well as finding ways to eradicate these mismatches. \et{can you get a number here? how many are in fact precision/strings issues?}

4.4\% of test cases end up classified as ``unknown'', which subsequently lead to potential passes and fails (4.2\%). Recall that these are due to the various ways in which GNU~R's pretty printer works, which \CoqR{} currently does not mimic properly. However, the relatively low percentage of these cases does not seriously question the validity of the implementation. Fixing these unknown results is tiresome, but not conceptually challenging.
...
Finally, it is worth highlighting that only 13 impossible results have been detected (0.06\%); these all correspond to actual bugs in \CoqR{}, which are already tracked and currently under scrutiny. 
