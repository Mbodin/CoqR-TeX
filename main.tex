...
(and they might become desynchronized).\mb{I think that the ``or'' was more correct here:
...
involved in the \CoqR{} project compared to the size of our interpreter,
...
the test suite of GNU~R itself (aprox.\ 3,600 lines)
and the test suite of FastR (aprox.\ 1,300 lines)~\parencite{kalibera2014fast}.
We found that these test suites mainly focus on expected behaviors.
For instance,
different numerical vectors will be concatenated,
summed, divided, etc.
In each case,
the special values \mintinline{R}{NaN} and \mintinline{R}{NA}
are checked to be propagated.
...
During our developpment,
we developped our own test suite (of approx.\ 1,100 lines),
looking for such corner cases.

% Tests are too various.
These tests come in various shapes.
Some files provide a set of expressions,
all of which supposed to evaluate to \mintinline{R}{TRUE}.
Some are just supposed not to raise any error.
Finally, some prints on the standard output
whether the test passed.
%
To uniformly test these test suites,
...
whilst some are supposed to be executed as a normal file,
some tests are supposed to be read line-by-line,
resetting the environment at each line.
...
We manually marked these special files by a \mintinline{R}{#@line}
comment, which can be easily detected by our testing framework.

% Differences of pretty-printer.
The pretty-printer of GNU~R is quite complex.
We did our best to match GNU~R's output on most cases,
but we do not consider this as part of our contributions:
\CoqR{}'s pretty-printer is part of the shim (the non-formalized part)
and does not follow any line-to-line correspondance with GNU~R's.
There are thus some differences of behavior.
Let us consider the expression
\mintinline{R}{x <- c (1, 100, 2); x}.
There are two steps of execution:
first the variant \mintinline{R}{x} is assigned a value,
then it is shown again.
In the first step,
the returned value of the assignment operator
is the assigned value, but is marked as ``invisible'': % I don't want to say about the parentheses trick: I like this example.
...
and we defined our testing framework to accept it as such.

% Testing principles.
To correctly compare each result,
we adopted the general scheme shown in Figure~\ref{fig:methodology}.
First,
we identify what type of result each interpreter returned. % I think that this is what Tomás calls ``Processor'', right?
Examples of types includes
errors, boolean vectors, numerical vectors, etc.,
but also ``not implemented'' and ``impossible''
(corresponding the the Coq constructors
\mintinline{Coq}{result_not_implemented} and \mintinline{Coq}{result_impossible}
shown in Figure~\ref{fig:result}).
These types are then compared.
For instance, if GNU~R returned an error,
but \CoqR{} returned a string vector,
the test is marked as failed.
The ``not implemented'' and ``impossible'' results
are special.
If \CoqR{} returns ``not implemented'',
we simply mark the test as not implemented
(this corresponds to an ``abandon'' in JSCert~\parencite{popl14jscert}).
If it returns ``impossible'',
it is directly marked as failed:
this would correspond to a critical bug in GNU~R and should imperatively be reported.
All of our ``impossible'' results
were due to a mistake in \CoqR{} and were quickly solved.

In the case where the two result types were identical
in both GNU~R and \CoqR{},
we interpret the results using a specialized function,
depending on the type previously computed.
For instance, the interpreter for numerical vectors
would convert both the results above
by the vector \mintinline{R}{c (1, 100, 2)},
ignoring the spaces.
The final result is then compared,
...
provide any interpreter.
For instance, there exist special attributes changing
the way vectors are printed,
and our pretty-printer is not aware of all of these special attributes.
In such cases, the typing step fails,
returning ``unknown'' as a type.
Such tests are marked as ``unknown''.
% it would not be correct to mark them as success,
% but it would not be practical to mark them as a failure.
%
As a special case, \mb{Do we want to speak about this here?}
...
Once a step in a test file failed or lead to a not-implemented result,
we can not say anything about what follows:
if we get the same result in both interpreter,
it could be out of luck.
They should not be marked as success:
otherwise, successful results could suddenly fail
after correctly implementing a feature.
%
Symmetrically,
if such a test fails,
it could be failing just because the previous step failed---%
for instance if the previous step was an assignment
who failed to execute
and the current step just displays the value of the assigned variable.
Marking such tests as failing would be counter-productive:
this would lead to a large amount of failed tests
that are not particularly failing,
but fails because of a previous failing result.
%
To categorize these tests,
we introduced two additional outcomes for tests:
potential success and potential failures.
These two categories mean nothing for correctness
(we can not conclude anything certain from a potential outcome),
but they really helped the development
by showing possible testing results that
implementing a feature could solve.
...
Figure~\ref{fig:testing:interface} shows an example of such
a categorization in action.
We found that it enables to quickly understand what parts
of the interpreter are needed to be fix or implemented.
%
As convenient feature of our framework consists
in counting how many times each not-implemented feature
was being needed during the execution of the test suites.
Figure~\ref{fig:interface:not:implemented} shows an example
of such a graph.
In practise, there were very often one function triggering
