...
Finally, it is worth highlighting that only 1 impossible result reamins in the current version, which corresponds to an actual bug in \CoqR{}, which is currently under scrutiny. 
...
given the right lemmae, but it would be cumbersome and repetitive.
%
...
We have proven that the invariants are safely propagated throughout the execution of several basic functions.
Some of these functions change the structures described by the invariants,
but eventually preserve the invariants. 
For instance, an element of the expected type is added to a shared list.\et{expand a bit: what's the invariant being preserved, and what is tricky about proving that it is preserved}
% The proof for the whole formalization is still work in progress.
Given the size of the formalization,\et{put size of proofs for now, considering X functions}
