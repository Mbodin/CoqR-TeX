...
%
\newtext{GNU~R's structure is common among dynamic-language interpreters:
we believe the principles exposed in this section to be reusable.}
...
\newtext{It behaves similarly to \mintinline{Coq}{result_error},
    but is caugth by the \mintinline{C}{setjmp} monadic binder,
    mimicking the C behavior.}
...
the continuation of \mintinline{C}{setjmp} is runned again,
the value carried by \mintinline{Coq}{result_longjump} replacing the default one.}
...
The other differences that can be observed are several arguments systematically passed to each function (\mintinline{Coq}{globals}, \mintinline{Coq}{runs}, and \mintinline{Coq}{S}),
the use of \mintinline{Coq}{read%list}
instead of \mintinline{C}{CAR} in the C version, as well as the fact that the use of the \mintinline{C}{PROTECT} macro is missing from the Coq version. We clarify each of these points in the following subsections.
...
\newtext{Various models of C's heap~\parencite{Leroy-Compcert-CACM, formalin} exist in Coq.
...
\newtext{and CoqR's heap reflects this}.
Each element is composed of a header and some data.
%
...
\newtext{\emph{Tactics} are programs manipulating Coq proof terms.
They are an important component of interactive theorem provers.}
...
\newtext{To define them, we proved about an hundred simple lemmas.
Our tactics then look in the context for patterns where these lemmas can be applied in a useful way:
...
Some lemmas require variables to be in a specific form:
some of our tactics have been specifically designed to rewrite the context into a form fitting specific lemmas.
We defined overall 80 tactics, most of which performing very simple operations on the proof context.}
...
\newtext{\CoqR{} uses a similar monadic structure than JSExplain~\parencite{JSExplain},
    making it possible to a build similar tool for R.}
