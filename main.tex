...
\eti{in this section blah blah blah}
...
\et{update description of test suites (including ours), link to originals: GNU~R, FastR-1, FastR-2, Custom}
...
Discriminating a failure \et{actual error, could be because of an unfound function} from a  not-yet-implemented feature is crucial for development, as it helps us choose which feature to focus on next (see Section~\ref{sec:missing}). \et{corresponds to C code, eg. features from symbol table}
%
...
Our testing framework can report the most common not-implemented features, allowing us to identify the most pressing functions to implement, as can be seen in Figure~\ref{fig:library:not_implemented_fastr}. By tackling these first, we can make progress in debugging \CoqR{} by running the test suites of GNU~R and FastR.


\et{expand, explain both kinds of charts, why they are useful, etc.}

not implemented = missing features from symbol table (mostly) but also some C code from the C interpreter that has not been (yet) included in \CoqR (typically example include skipping a branch from a large switch)

missing function definitions = use of an undefined R function in the test
(could be because not imported). This brought us to focus on the (some parts of the) base library first. 

Also useful for scalability: only import needed functions from base library
(Coq is slow!)
