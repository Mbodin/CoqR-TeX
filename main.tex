...
The monad constructors are:\footnote{The result monad is similar to the one used in the JSExplain project~\parencite{JSExplain}, which aims at defining a JavaScript interpreter in Coq, readable by non-specialists.
}
\begin{itemize}
...
% Other works on R.
\newtext{
Previous works on formal specification of R are sparse.
%
We have seen in the introduction that the language definition document~\parencite{R}
is unfit for verification due to its ambiguities,
as well as several mismatches between the document and the behaviour of GNU~R.
%
The only other work we found about formalising R~\parencite{morandat2012evaluating}
only describes a small core of R
containing functions and some assignments---%
but no flow-control constructs like \mintinline{R}{if} or \mintinline{R}{return}
and their associated implicit type conversions.
}
%
To the best of our knowledge, we are the first to provide a mechanized specification of R.
