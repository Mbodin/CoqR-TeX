...
them in the \CoqR{} environment does not mean that these definitions are usable: the internal function called by a library function may not yet be implemented in \CoqR, so if a test executes that function, it will yield a not-implemented result. This is why it is crucial to be able to differentiate real failures from not-implemented results. Our testing framework can report the most common not-implemented features, allowing us to identify the most pressing functions to implement, as can be seen in Figure~\ref{fig:library:missing_functions}. By tackling these first, we can make progress in debugging \CoqR{} by running the test suites of GNU~R and FastR.

\et{refine/extend/etc.}
Indeed, the base library is crucial in order to execute the test suites of both GNU~R and FastR. For the GNU~R (resp. Faster) test suite, more than 80\% (resp. 60\%) of the failed results are due to missing definitions
