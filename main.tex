...
We thus developped our own test suite (of approx.\ 1,100 lines),
looking for such corner cases.

% Tests are too various.
These tests come in various shapes.
Some files provide expressions supposed to evaluate to \mintinline{R}{TRUE}.
...
We manually marked these special files by a \mintinline{R}{#@line} comment,
which is recognized by our testing framework.

% Differences of pretty-printer.
The pretty-printer of GNU~R is quite complex.
We did our best to match GNU~R's output in most cases,
...
(corresponding the Coq constructors
...
but fails because of a previous failing result.\mb{too much false positive.}
%
...
by showing possible future results.
...
%
\mb{What follows should probably be moved to Section~\ref{sec:library:development}}
Our framework also counts how many times each not-implemented feature
was being needed in our test suites.
Figure~\ref{fig:interface:not:implemented} shows a typical example
of such a graph given by our framework.
One function here triggers
most of the not-implemented results:
this often happenned in practise
and indicates that this function is being used
in a lot of places in the test suites.
