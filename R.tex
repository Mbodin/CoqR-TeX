...
constant global variables that need initialisation:
\begin{minted}{Coq}
Record Globals := {
...
is parameterised by such a \mintinline{Coq}{Globals}
objects.
An instance of this object is defined once every function has been defined,
all parameterised by a \mintinline{Coq}{runs} structure,
and some parameterised by a \mintinline{Coq}{Globals} structure.
...
The definition of \mintinline{Coq}{Globals}
is more complex than what it may seem.
In practise, this structure is very large,
and the initialisation procedure is subtle.
...
But updating each fields of the large record \mintinline{Coq}{Globals}
in \Coq{} is quite complicated:
one has to either define an updating function for each of its projection,
or write a large definition of the form shown below.
\begin{minted}{Coq}
  {| R_NilValue := (* new value *) ;
     R_TrueValue := (* old value *) ;
     (* ... *) |}
\end{minted}
The first solution is not a realistic option:
when adding a field,
this solution requires to update \emph{all} already defined updating functions
to add the new field,
then add one new function.
The \mintinline{Coq}{Globals} structure has approximately an hundred fields,
which would result in an hundred definitions to be updated at each changed,
and resulting in about ten thousands (the square) of \Coq{} lines.
It is too easy to make a mistake in such a number of lines.
The second solution is unfortunately also not an option:
this would require to define similarly large definitions
in the initialising function,
resulting in large use of copy/pasting.
The trust of such a formalisation would be far too weak.
Here is the code of the initialising function in \Coq:
\begin{minted}{Coq}
...
\end{minted}
As can be seen, we have used neither of both solution.
Instead, we have developed a notation (similar to \OCaml{}'s)
to update the \mintinline{Coq}{Globals} structure.
The signature of this notation is shown below.
In a nutshell, it builds using tactics
an object of the form \mintinline{Coq}/{ g' | globals_with_list g L g' }/,
then takes its projection \mintinline{Coq}{proj1_sig}.
This guarantees, whatever the trust we have on these tactics,
that the constructed object \mintinline{Coq}{g'} verifies
the property \mintinline{Coq}{globals_with_list g L g'}.
This property is defined exactly as expected:
for each projection in the given list \mintinline{Coq}{L},
the value of \mintinline{Coq}{g'} has been updated to the given value;
for all other projections, the value has not been changed.
\begin{minted}{Coq}
Record globals_with_list g
  (L : list ((Globals -> SExpRec_pointer) * SExpRec_pointer)) g' := {
...
Definition Globals_with_list (g : Globals)
  (L : list ((Globals -> SExpRec_pointer) * SExpRec_pointer)) :
...
Notation "'{{' g 'with' L '}}'" :=
  (proj1_sig (Globals_with_list g L
              ltac:(prove_mem_Globals_projections),
              ltac:(prove_no_duplicate_projections)}})) : globals_scope.
\end{minted}
These tactics are fairly complicated,
and a great care has been done to have them run in an acceptable
amount of time and memory:
\Coq{} tactics can be pretty slow,
and the first implementations only worked for few number of fields
(less than ten) given our computer's memory.
The implementation of these tactics can be found in the file \texttt{low/Globals.v}
