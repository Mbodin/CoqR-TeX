...
\sectionframe**{CoqR}{
    ...
    \begin{itemize}
        ...
        \item \anchorm{l18Kbegin}{}\emph{18,000 lines}\anchorm{l18Kend}{} of Coq definitions;
        % \item A \emph{runnable} semantics;
        \item Supports a non-trivial subset of R, and \emph{fully} support them.
    \end{itemize}
...
\sectionframe{Testing Framework}
...
\begin{frame}
    ...
    \begin{centertikz}
        ...
        \onslide<2->{
            \node [below = 25mm of coqR] (Rlibraries) {R base library (R)} ;
        }
        ...
        \draw<2-> [<-, DarkPlum, thick] (Rlibraries) to [bend left]
            node [sloped, above, DarkPlum] {execute} (coqR) ;
        \draw<2-> [->, DarkSkyBlue, thick] (Rlibraries) to [bend right]
            node [sloped, above, DarkSkyBlue] {Initial state} (coqR) ;
        \draw [<-, DarkPlum, thick] (tests) -- node [sloped, below, DarkPlum] {execute} (coqR) ;
